...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Pelaporan Isidentil</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_pelaporan_isidentil')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <table id="pelaporan_isidentil" class="table table-bordered table-striped table-hover table-responsive pelaporan_isidentil">
                <thead>
                    <th>No</th>
                    <th>Tanggal Input Data</th>
                    <th>Jenis Pelaporan</th>
                    <th>Pihak Yang Menerima Laporan</th>
                    <th>Perihal Laporan</th>
...
@includeIf('pelaporan.pelaporan_isidentil.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#pelaporan_isidentil").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('pelaporan_isidentil.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'tanggal_input_data' },
                { data: 'jenis_pelaporan' },
                { data: 'pihak_menerima' },
                { data: 'perihal_laporan' },
                {
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/pelaporan/pelaporan_isidentil') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/pelaporan/pelaporan_isidentil') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#pelaporan_isidentil_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Pelaporan Isidentil');
        ...
        .done((response) => {
            ...
            $('#modal-form [name=jenis_pelaporan]').attr('disabled', false);
            $('#modal-form [name=pihak_menerima]').attr('disabled', false);
            $('#modal-form [name=perihal_laporan]').attr('disabled', false);
        })
        .fail((errors) => {
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Pelaporan Isidentil');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=jenis_pelaporan]').val(response.jenis_pelaporan).attr('disabled', false);
            $('#modal-form [name=pihak_menerima]').val(response.pihak_menerima).attr('disabled', false);
            $('#modal-form [name=perihal_laporan]').val(response.perihal_laporan).attr('disabled', false);
        })
        .fail((errors) => {
            alert('Cannot display data');
        });
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Pelaporan Isidentil');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=jenis_pelaporan]').val(response.jenis_pelaporan).attr('disabled', true);
            $('#modal-form [name=pihak_menerima]').val(response.pihak_menerima).attr('disabled', true);
            $('#modal-form [name=perihal_laporan]').val(response.perihal_laporan).attr('disabled', true);
        })
        .fail((errors) => {
            alert('Cannot display data');
        });
