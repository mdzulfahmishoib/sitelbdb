...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Pelaporan Pemkab Bojonegoro</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_pelaporan_pemkab')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <table id="pelaporan_pemkab" class="table table-bordered table-striped table-hover table-responsive pelaporan_pemkab">
                <thead>
                    <th>No</th>
                    ...
                    <th>Instansi yang Dilaporkan</th>
...
@includeIf('pelaporan.pelaporan_regulasi.pemkab.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#pelaporan_pemkab").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('pelaporan_pemkab.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_pemkab') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_pemkab') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#pelaporan_pemkab_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Pelaporan PEMKAB');
        ...
        .done((response) => {
            ...
            $('#modal-form [name=instansi_dilaporkan]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Pelaporan PEMKAB');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=instansi_dilaporkan]').val(response.instansi_dilaporkan).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Pelaporan PEMKAB');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=instansi_dilaporkan]').val(response.instansi_dilaporkan).attr('disabled', true);
