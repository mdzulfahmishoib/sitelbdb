...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Pelaporan DIRJEN PAJAK</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_pelaporan_dirjen_pajak')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <table id="pelaporan_dirjen_pajak" class="table table-bordered table-striped table-hover table-responsive pelaporan_dirjen_pajak">
                <thead>
                    <th>No</th>
...
@includeIf('pelaporan.pelaporan_regulasi.dirjen_pajak.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#pelaporan_dirjen_pajak").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('pelaporan_dirjen_pajak.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_dirjen_pajak') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_dirjen_pajak') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#pelaporan_dirjen_pajak_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Pelaporan DIRJEN PAJAK');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Pelaporan DIRJEN PAJAK');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Pelaporan DIRJEN PAJAK');
