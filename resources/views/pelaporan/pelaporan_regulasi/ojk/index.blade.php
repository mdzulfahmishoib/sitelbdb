...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Pelaporan OJK</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_pelaporan_ojk')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <table id="pelaporan_ojk" class="table table-bordered table-striped table-hover table-responsive pelaporan_ojk">
                <thead>
                    <th>No</th>
...
@includeIf('pelaporan.pelaporan_regulasi.ojk.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#pelaporan_ojk").DataTable({
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_ojk') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_ojk') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#pelaporan_ojk_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Pelaporan OJK');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Pelaporan OJK');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Pelaporan OJK');
