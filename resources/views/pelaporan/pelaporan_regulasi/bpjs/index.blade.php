...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Pelaporan BPJS</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            @can('create_pelaporan_bpjs')
            <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                Data</button>
            @endcan
            <table id="pelaporan_bpjs" class="table table-bordered table-striped table-hover table-responsive pelaporan_bpjs">
                <thead>
                    <th>No</th>
...
@includeIf('pelaporan.pelaporan_regulasi.bpjs.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#pelaporan_bpjs").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_bpjs') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/pelaporan/pelaporan_regulasi/pelaporan_bpjs') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#pelaporan_bpjs_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Pelaporan BPJS');
        ...
        .done((response) => {
            ...
            $('#modal-form [name=jenis_bpjs]').attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Pelaporan BPJS');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=jenis_bpjs]').val(response.jenis_bpjs).attr('disabled', false);
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Pelaporan BPJS');
    ...
    $.get(url)
        .done((response) => {
            ...
            $('#modal-form [name=jenis_bpjs]').val(response.jenis_bpjs).attr('disabled', true);
