...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Kategori Produk Kredit</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            <div class="btn-group">
                @can('create_kategori_produk_kredit')
                   <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                    Kategori</button> 
                @endcan
                <a href="{{ route('produk_kredit.index') }}" class="btn btn-warning mb-2"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
            </div>
            <table id="kategori_produk_kredit" class="table table-bordered table-striped table-hover table-responsive kategori_produk_kredit">
                <thead>
                    <th>No</th>
                    <th>Judul</th>
                    <th>Deskripsi</th>
...
@includeIf('produk_perusahaan.kredit.kategori_produk_kredit.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#kategori_produk_kredit").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('kategori_produk_kredit.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'judul' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/produk_perusahaan/kredit/kategori_produk_kredit') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/produk_perusahaan/kredit/kategori_produk_kredit') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#kategori_produk_kredit_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Produk Kredit');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Produk Kredit');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Produk Kredit');
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Produk Kredit');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("kategori_produk_kredit.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
    ...
    var fullImageUrl = '{{ asset('storage/produk_perusahaan/kredit/kategori_produk_kredit') }}/' + imageUrl;
