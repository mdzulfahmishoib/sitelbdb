...
@section('title')
    Kategori Produk Mobile Banking
@endsection

@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Kategori Produk Mobile Banking</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            <div class="btn-group">
                @can('create_kategori_produk_mobile_banking')
                   <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                    Kategori</button> 
                @endcan
                <a href="{{ route('produk_mobile_banking.index') }}" class="btn btn-warning mb-2"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
            </div>
            <table id="kategori_produk_mobile_banking" class="table table-bordered table-striped table-hover table-responsive kategori_produk_mobile_banking">
                <thead>
                    <th>No</th>
                    <th>Judul</th>
                    <th>Deskripsi</th>
...
@includeIf('produk_perusahaan.mobile_banking.kategori_produk_mobile_banking.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#kategori_produk_mobile_banking").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('kategori_produk_mobile_banking.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'judul' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/produk_perusahaan/mobile_banking/kategori_produk_mobile_banking') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/produk_perusahaan/mobile_banking/kategori_produk_mobile_banking') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#kategori_produk_mobile_banking_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Produk Mobile Banking');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Produk Mobile Banking');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Produk Mobile Banking');
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Produk Mobile Banking');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("kategori_produk_mobile_banking.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
    ...
    var fullImageUrl = '{{ asset('storage/produk_perusahaan/mobile_banking/kategori_produk_mobile_banking') }}/' + imageUrl;
