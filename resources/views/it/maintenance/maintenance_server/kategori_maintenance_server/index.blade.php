...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Kategori Maintenance Server</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            <div class="btn-group">
                <a href="{{ route('maintenance_server.index') }}" class="btn btn-warning mb-2"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
                <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                    Kategori</button>
            </div>
            <table id="kategori_maintenance_server" class="table table-bordered table-striped table-hover table-responsive kategori_maintenance_server">
                <thead>
                    <th>No</th>
                    <th>Nama Kategori</th>
...
@includeIf('it.maintenance.maintenance_server.kategori_maintenance_server.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#kategori_maintenance_server").DataTable({
            "paging": true,
            "lengthChange": false,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('kategori_maintenance_server.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'kategori_maintenance_server' },
                { data: 'aksi', searchable: false, sortable: false },
            ],
            "initComplete": function() {
                table.buttons().container().appendTo('#kategori_maintenance_server_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Kategori Maintenance Server');
    ...
    function editForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Edit Kategori Maintenance Server');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=kategori_maintenance_server]').val(response.kategori_maintenance_server);
