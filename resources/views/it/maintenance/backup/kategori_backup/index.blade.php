...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Kategori Backup</li>
@endsection
...
<section class="content">
    ...
    <div class="card">
        <div class="card-body">
            <div class="btn-group">
                <a href="{{ route('backup.index') }}" class="btn btn-warning mb-2"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
                <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                    Kategori</button>
            </div>
            <table id="kategori_backup" class="table table-bordered table-striped table-hover table-responsive kategori_backup">
                <thead>
                    <th>No</th>
                    <th>Nama Kategori</th>
...
@includeIf('it.maintenance.backup.kategori_backup.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#kategori_backup").DataTable({
            "paging": true,
            "lengthChange": false,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('kategori_backup.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#kategori_backup_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Kategori Backup');
    ...
    function editForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Edit Data Kategori Backup');
    ...
    $.get(url)
        .done((response) => {
            $('#modal-form [name=kategori_backup]').val(response.kategori_backup);
