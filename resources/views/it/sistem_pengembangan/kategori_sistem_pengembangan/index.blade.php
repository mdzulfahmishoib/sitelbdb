...
@section('title')
    Kategori Sistem Pengembangan
@endsection

@section('breadcrumb')
    @parent
    <li class="breadcrumb-item active">Kategori Sistem Pengembangan</li>
@endsection
            ...
            <div class="btn-group">
                @can('create_sistem_pengembangan')
                   <button onclick="addForm()" class="btn btn-primary mb-2"><i class="fa fa-plus-circle"></i> Tambah
                    Kategori</button> 
                @endcan
                <a href="{{ route('sistem_pengembangan.index') }}" class="btn btn-warning mb-2"><i class="fa fa-arrow-circle-left"></i> Kembali</a>
            </div>
            <table id="kategori_sistem_pengembangan" class="table table-bordered table-striped table-hover table-responsive kategori_sistem_pengembangan">
                <thead>
                    <th>No</th>
                    <th>Nama</th>
                    <th>Deskripsi</th>
...
@includeIf('it.sistem_pengembangan.kategori_sistem_pengembangan.form')
@endsection
...
var table;
    ...
    $(function () {
        table = $("#kategori_sistem_pengembangan").DataTable({
            "paging": true,
            "lengthChange": true,
            "searching": true,
            "ordering": true,
            ...
            ajax: {
                url: '{{ route('kategori_sistem_pengembangan.data') }}',
            },
            columns: [
                { data: 'DT_RowIndex' },
                { data: 'judul' },
                    ...
                    render: function(data, type, full, meta) {
                        ...
                        var htmlOutput = images.map(function(file) {
                            ...
                            if (imageExtensions.includes(fileExtension)) {
                                ...
                                return '<img src="{{ asset('storage/it/sistem_pengembangan/kategori_sistem_pengembangan') }}/' + file + '" height="50" class="mb-1" title="Dokumentasi" style="cursor:pointer;" onclick="showImageModal(\'' + file + '\')"/>';
                            } else {
                                // Jika file bukan gambar, tampilkan tombol download
                                return '<a href="{{ asset('storage/it/sistem_pengembangan/kategori_sistem_pengembangan') }}/' + file + '" class="btn btn-primary" title="Download">Download</a>';
                            }
                        }).join(' ');
            ...
            "initComplete": function() {
                table.buttons().container().appendTo('#kategori_sistem_pengembangan_wrapper .col-md-6:eq(0)');
            }
        });
    ...
    function addForm(url) {
        ...
        $('#modal-form .modal-title').text('Tambah Data Sistem Pengembangan');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Edit Data Sistem Pengembangan');
    ...
    $('#modal-form').modal('show');
    $('#modal-form .modal-title').text('Lihat Data Sistem Pengembangan');
    ...
    function cloneForm(url) {
        $('#modal-form').modal('show');
        $('#modal-form .modal-title').text('Clone Data Sistem Pengembangan');

        // Reset form
        $('#modal-form form')[0].reset();
        $('#modal-form form').attr('action', '{{ route("kategori_sistem_pengembangan.store") }}'); // Gunakan route store untuk menyimpan data baru
        $('#form-method').val('POST');
        $('#submit-button').text('Simpan');
    ...
    var fullImageUrl = '{{ asset('storage/it/sistem_pengembangan/kategori_sistem_pengembangan') }}/' + imageUrl;
